...
def test_simulate_feature_drift_changes_expected_features():
    ...
    assert drifted_df["f0"].tolist() == pytest.approx([1.5, 3.0, 4.5])
    assert drifted_df["f3"].tolist() == pytest.approx([1.2, 2.2, 3.2])
    assert drifted_df["f7"].tolist() == pytest.approx([0.7, 1.4, 2.1])