...
export function MonitoringPage() {
  ...
  return <ShellPage active="/monitoring" intro={{ eyebrow: "MONITORING", title: "Know when the platform needs attention.", description: "Monitor service availability, decision traffic, and whether incoming customer data still resembles the training reference." }} onRefresh={load} overview={overview} refreshing={refreshing}><GlobalError error={error} retry={load} /><div className="metric-grid monitoring-metrics">{["request_rate", "error_rate", "average_latency_seconds", "average_uplift", "average_expected_value"].map((key) => { const meta = METRIC_LABELS[key] ?? { label: key, unit: "value", description: "Platform signal." }; return <MetricCard key={key} label={meta.label} value={formatNumber(overview?.metrics[key])} unit={meta.unit} description={meta.description} />; })}</div><div className="content-grid two-column"><Panel><PanelHeading eyebrow="PRODUCTION DRIFT" title="Input-data change" />{drift?.available ? <><div className={`signal-banner ${drift.should_retrain ? "warning" : "success"}`}><strong>{drift.should_retrain ? "Review recommended" : "No action recommended"}</strong><span>{drift.n_drifted_features ?? 0} of {drift.n_features ?? 11} tracked attributes changed materially.</span></div><p className="panel-description">Production drift means that recent customer data has a different distribution from the reference data used to assess the model. It is a signal to investigate, not an automatic retraining command.</p>{drift.retrain_reasons?.length ? <div className="reason-list"><span className="muted">Why it was flagged</span>{drift.retrain_reasons.map((reason) => <span key={reason}>• {reasonLabel(reason)}</span>)}</div> : null}{drift.report_url && <a href={drift.report_url} target="_blank" rel="noreferrer">Open the detailed drift summary ↗</a>}</> : <StatusMessage kind="info">No production drift report is available yet. Use Operations when you explicitly want to analyze production data.</StatusMessage>}</Panel><Panel><PanelHeading eyebrow="SERVICE HEALTH" title="Where the platform stands" />{overview?.services.map((service) => <div className="service-row" key={service.name}><span className={`status-dot ${service.status}`} /><strong>{serviceLabel(service.name)}</strong><span className="muted service-detail">{service.status === "healthy" ? "Available" : "Needs attention"}</span></div>)}<SpecialistLinks links={overview?.links ?? {}} include={["grafana", "prometheus"]} /></Panel><DriftSimulatorPanel /></div></ShellPage>;
}

export function SimulationLabPage() {
  const { overview, error, refreshing, load } = useOverview();
  return <ShellPage active="/simulation-lab" intro={{ eyebrow: "SIMULATION LAB", title: "See how the champion model responds to change.", description: "Create a safe synthetic scenario, compare predictions before and after the change, and download the paired result when you need it." }} onRefresh={load} overview={overview} refreshing={refreshing}><GlobalError error={error} retry={load} /><DriftSimulatorPanel modelReady={Boolean(overview?.model.model_loaded)} /></ShellPage>;
}
